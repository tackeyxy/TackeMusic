...
class MusicRecognizerApi {
    ...
    /**
     * 识别音频指纹
     * @param fingerprint Base64编码的音频指纹
     * @param duration 音频时长（秒）
     * @return 识别结果列表，失败返回null
     */
    suspend fun recognize(fingerprint: String, duration: Int = 3): List<SongResult>? = withContext(Dispatchers.IO) {
        try {
            ...
            val fullUrl = buildString {
                append(url)
                append("?")
                append(queryParams.entries.joinToString("&") {
            ...
            connection.setRequestProperty(
                "User-Agent",
                "Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.74 Mobile Safari/537.36"
            )
